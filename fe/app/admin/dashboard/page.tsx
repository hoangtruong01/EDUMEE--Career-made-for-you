  ...
  return (
    <div className="w-full">
      ...
      <AdminSectionHeader
        ...
        right={
          <div className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-500 dark:text-slate-400 shadow-sm">
              ...
              <button
                ...
                className={cn(
                  ...
                  activityFilter === filter.value
                    ? 'border-violet-200 dark:border-violet-500/30 bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400 shadow-sm shadow-violet-500/5'
          ...
          <div className="space-y-3">
            {data?.popularCareers.map((item, index) => (
              <div key={item.name} className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`text-xs font-semibold ${item.delta.startsWith('-') ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}
                >
                  {item.delta}
                </span>
              </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {quickActions.map((item) => (
          <button
            ...
            className="flex items-center gap-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-4 text-sm font-semibold text-slate-800 dark:text-slate-200 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:hover:border-violet-500/30"
          >
            <span
              className={`flex h-9 w-9 items-center justify-center rounded-xl text-white ${item.color} shadow-lg shadow-black/5`}
            >
              <item.icon className="h-4 w-4" />
            </span>
            {item.label}
            <Plus className="ml-auto h-4 w-4 text-slate-400 dark:text-slate-500" />
          </button>
        ))}
      </div>
