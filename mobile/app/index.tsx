...
import { getAuthToken, setAuthToken } from '../src/services/api';
...
export default function WelcomeScreen() {
  ...
  useEffect(() => {
    const verifySession = async () => {
      try {
        ...
        console.error('Verify session status error:', err);
        setIsCheckingAuth(false);
      }
    };
    verifySession();
  ...
  // Staggered Waterfall Entrance (Chỉ kích hoạt khi đã check xong token)
  useEffect(() => {
    if (!isCheckingAuth) {
  ...
  useEffect(() => {
    if (!isCheckingAuth) {
      Animated.loop(
        Animated.sequence([
          Animated.timing(scalePulse, { toValue: 1.03, duration: 1000, useNativeDriver: true }),
  ...
  }, [charIndex, isDeleting, wordIndex, isCheckingAuth]);
        ...
        <GlassView style={styles.topHeader} intensity={25} tint="dark">
          <View style={styles.topHeaderContent}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [styles.topLoginBtn, { opacity: pressed ? 0.7 : 1 }]}
            >
        ...
        >
          ...
          >
            ...
            <Text style={styles.heroDesc}>
              AI phân tích tính cách & sở thích cá nhân, gợi ý ngành nghề phù hợp kèm lộ trình học
              tập tối ưu – hoàn toàn miễn phí.
            </Text>

            <Animated.View style={{ width: '100%', transform: [{ scale: scalePulse }] }}>
              <Pressable
                onPress={() => router.push('/register')}
                style={({ pressed }) => [styles.primaryCtaButton, { opacity: pressed ? 0.9 : 1 }]}
              >
          ...
          <Animated.View
            style={{ opacity: featuresFade, transform: [{ translateY: featuresSlide }] }}
          >
            <View style={styles.sectionHeader}>
            ...
            <View style={styles.bentoGridContainer}>
              <GlassView style={styles.bentoLargeCard}>
                ...
                <Text style={styles.bentoLargeDesc}>
                  Thuật toán AI độc quyền phân tích sâu 24 đặc điểm tính cách, dự đoán độ tương
                  thích với 500+ ngành nghề khác nhau với độ chính xác 98%.
                </Text>
                <View style={styles.bentoBadgesRow}>
                  <View style={styles.bentoBadge}>
                    <Text style={styles.bentoBadgeText}>NLP Analysis</Text>
                  </View>
                  <View style={styles.bentoBadge}>
                    <Text style={styles.bentoBadgeText}>Predictive Modeling</Text>
                  </View>
                </View>
              </GlassView>
          ...
          <Animated.View style={{ opacity: testFade, transform: [{ translateY: testSlide }] }}>
            ...
            <Animated.View style={{ opacity: testimonialCardFade }}>
              <GlassView style={styles.socialProofCard}>
                ...
                <Text style={styles.socialBody}>{TESTIMONIALS[activeTestimonial].text}</Text>
                ...
                >
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          activeTestimonial === idx ? COLORS.primary : 'rgba(255, 255, 255, 0.2)',
...
const styles = StyleSheet.create({
  ...
  glowOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(15, 23, 42, 0.82)' },
  topHeader: {
    position: 'absolute',
  ...
  brandTitle: { fontSize: 20, fontWeight: '900', color: COLORS.foreground, letterSpacing: 2.5 },
  topLoginBtn: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  topLoginText: { color: COLORS.primary, fontSize: 13, fontWeight: '700' },
  scrollContent: {
    paddingHorizontal: SPACING.lg,
    paddingTop: Platform.OS === 'ios' ? 120 : 100,
    paddingBottom: 40,
  },
  heroContainer: { alignItems: 'center', marginBottom: SPACING.xl, marginTop: SPACING.sm },
  tagPill: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: 'rgba(59, 130, 246, 0.1)',
  ...
  tagText: { color: '#60A5FA', fontSize: 11, fontWeight: '700' },
  subTitle: { fontSize: 22, fontWeight: '700', color: COLORS.foreground, textAlign: 'center' },
  ...
  rotatingTextPrefix: { fontSize: 15, color: COLORS.muted, fontWeight: '600' },
  rotatingTextActive: { fontSize: 15, color: COLORS.secondary, fontWeight: '800' },
  cursor: { fontSize: 15, color: COLORS.secondary, fontWeight: 'bold' },
  heroDesc: {
    fontSize: 13,
    color: COLORS.muted,
    textAlign: 'center',
  ...
  heroNoticeRow: { flexDirection: 'row', alignItems: 'center', marginTop: SPACING.md },
  heroNoticeText: { fontSize: 11, color: COLORS.muted, fontWeight: '500' },
  statsSection: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    gap: SPACING.sm,
  ...
  statCard: { flex: 1, paddingVertical: SPACING.md, alignItems: 'center', borderRadius: RADIUS.lg },
  statIconWrapper: {
    width: 36,
    height: 36,
    borderRadius: RADIUS.md,
  ...
  sectionHeader: { alignItems: 'center', marginBottom: SPACING.md, marginTop: SPACING.md },
  smallTag: {
    backgroundColor: 'rgba(59, 130, 246, 0.1)',
    borderWidth: 1,
    borderColor: 'rgba(59, 130, 246, 0.2)',
  ...
  smallTagText: { color: '#60A5FA', fontSize: 10, fontWeight: '700', letterSpacing: 1 },
  smallTagTextPurple: { color: '#A78BFA', fontSize: 10, fontWeight: '700', letterSpacing: 1 },
  sectionTitle: {
    fontSize: 20,
    fontWeight: '800',
  ...
  stepsVerticalContainer: { gap: SPACING.md, marginBottom: SPACING.xxl },
  stepCardVertical: {
    padding: SPACING.lg,
  ...
  stepNumber: { fontSize: 32, fontWeight: '900', color: 'rgba(255, 255, 255, 0.08)' },
  stepTitle: {
    fontSize: 16,
    fontWeight: '800',
    color: COLORS.foreground,
  ...
  stepDesc: { fontSize: 12, color: COLORS.muted, lineHeight: 18 },
  bentoSectionTitle: {
    fontSize: 24,
    fontWeight: '800',
    color: COLORS.foreground,
  ...
  bentoGridContainer: { gap: SPACING.md, marginBottom: SPACING.xxl },
  bentoLargeCard: {
    padding: SPACING.lg,
    borderRadius: RADIUS.xl,
  ...
  bentoLargeHeader: { flexDirection: 'row', justifyContent: 'flex-end', marginBottom: SPACING.sm },
  bentoIconBlueSquare: {
    width: 48,
    height: 48,
    borderRadius: RADIUS.md,
  ...
  bentoLargeDesc: { fontSize: 13, color: COLORS.muted, lineHeight: 19, marginBottom: SPACING.md },
  bentoBadgesRow: { flexDirection: 'row', gap: SPACING.sm, marginTop: SPACING.xs },
  bentoBadge: {
    backgroundColor: 'rgba(255, 255, 255, 0.04)',
    borderWidth: 1,
  ...
  bentoBadgeText: { color: COLORS.muted, fontSize: 10, fontWeight: '600' },
  ...
  bentoSmallDesc: { fontSize: 11, color: COLORS.muted, lineHeight: 16 },
  socialProofCard: {
    padding: SPACING.lg,
    borderRadius: RADIUS.xl,
    backgroundColor: 'rgba(255, 255, 255, 0.03)',
  ...
  socialHeaderRow: { flexDirection: 'row', alignItems: 'center', marginBottom: SPACING.md },
  socialTitle: { fontSize: 14, fontWeight: '700', color: COLORS.secondary, flex: 1 },
  ...
  socialAuthorRow: { flexDirection: 'row', alignItems: 'center' },
  authorAvatar: {
    width: 36,
    height: 36,
    borderRadius: 18,
  ...
  authorSub: { fontSize: 11, color: COLORS.muted, marginTop: 1 },
  dotsRow: {
    flexDirection: 'row',
    justifyContent: 'center',
    alignItems: 'center',
