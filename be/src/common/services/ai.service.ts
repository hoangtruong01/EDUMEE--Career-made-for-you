// be/src/modules/careers/services/ai.service.ts
import { Injectable, Logger } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';

import { CareerSimulationData } from '../../modules/careers/schemas/career-simulation.schema';
import { AIAnalysisResult, AssessmentAnswerData } from '../interfaces/ai-analysis.interface';
...
export interface DetailedCareerAnalysis {
  ...
  pros: string[];
  cons: string[];
  trends: Array<{ year: string; description: string }>;
  salaryRange: string;
  demandLevel: string;
  keySkills: string[];
  topCompanies: string[];
...
export class AIService {
  ...
  private readonly model = 'models/gemini-2.5-flash-lite';
  ...
  async analyzePersonalityAndCareers(
    ...
    try {
      ...
      const response = await this.callGeminiAPI(prompt, 3, true);
...
`;
  ...
  public async callGeminiAPI(prompt: string, retries = 3, isJson = true): Promise<string> {
    if (!this.apiKey) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (this.isRetryableGeminiStatus(response.status) && attempt < retries) {
            ...
            this.logger.warn(
              `Retryable Gemini error (${response.status}). Retrying in ${waitTime}ms...`,
            );
    ...
    return {
      personalityAnalysis: {
        ...
        personalityProfile: {
          ...
          workStyle: 'Linh hoạt với nhiều môi trường làm việc Harvard',
      ...
      careerRecommendations: [
        {
          ...
          fitScore: 75,
          personalityMatch: { bigFiveAlignment: 70, riasecAlignment: 80, overallFit: 75 },
...
`;
    ...
    try {
      const response = await this.callGeminiAPI(prompt, 3, false);
      return response;
    } catch (error) {
  ...
  async generateDetailedCareerAnalysis(
    careerTitle: string,
    personalityTraits: string[],
  ): Promise<DetailedCareerAnalysis> {
    const prompt = `
...
`;
    ...
    try {
      ...
      return {
        ...
        cons: [
          'Đòi hỏi học hỏi liên tục',
          'Áp lực công việc cao',
          'Cạnh tranh trong ngành ngày càng tăng',
          'Cần cập nhật kỹ năng thường xuyên',
        ],
        trends: [
...
`;
            ...
            milestones: [
              {
                ...
                tasks: [
                  {
                    taskId: 't1_1_1',
                    taskTitle: 'Nghiên cứu tổng quan về ngành',
                    isRequired: true,
                    estimatedHours: 8,
                    order: 1,
                  },
                  {
                    taskId: 't1_1_2',
                    taskTitle: 'Học các khái niệm cơ bản',
                    isRequired: true,
                    estimatedHours: 16,
                    order: 2,
                  },
                  {
                    taskId: 't1_1_3',
                    taskTitle: 'Thực hành bài tập cơ bản',
                    isRequired: true,
                    estimatedHours: 12,
                    order: 3,
                  },
  ...
  async generateCareerSimulation(
    careerTitle: string,
    personalityTraits: string[],
  ): Promise<CareerSimulationData> {
    const prompt = `
      Hãy tạo một bản mô phỏng lộ trình nghề nghiệp chi tiết cho nghề: "${careerTitle}".
      Dựa trên đặc điểm tính cách của người dùng: ${personalityTraits.join(', ')}.
      Cấu hình mảng levels gồm: Intern, Junior, Senior, và Lead/Manager.
  ...
  private createFallbackCareerSimulation(careerTitle: string): CareerSimulationData {
    return {
      ...
      levels: [
        {
          label: `Thực tập sinh ${careerTitle}`,
          salaryRange: '3-6 triệu VND',
          yearRange: '0-1 năm',
          dailyTasks: [
            `Tìm hiểu quy trình làm việc của nghề ${careerTitle}`,
            'Hỗ trợ tài liệu chuyên môn',
          ],
          typicalSchedule: [
  ...
  async generateFullCareerData(title: string): Promise<Record<string, unknown>> {
    const prompt = `
Hãy tạo hồ sơ nghề nghiệp phân tích thị trường lao động cho nghề: "${title}" tại thị trường Việt Nam.
Trả về đối tượng JSON phẳng không giải thích văn bản thừa.
`;
